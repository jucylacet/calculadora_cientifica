...
writeToDisplay(data) {
    let legacy = document.getElementById("displayTela").value;
    if (data == ".") {
        legacy += data;
    } else {
        legacy = legacy == "0" ? data : legacy += data;
    }
    document.getElementById("displayTela").value = legacy;
}
...
clearDisplay() {
    document.getElementById("displayTela").value = "0";
}

solveOperation() {
    let operation = document.getElementById("displayTela").value;
    let result = 0;
    try {
    ...
    document.getElementById("displayTela").value = result;
    return result;
}
}
...
class CalculadoraCientifica extends CalculadoraBasica {
    ...
    writeToDisplay(data) {
        if (document.getElementById("displayTela").value == "Syntax Error") {
            super.clearDisplay();
        }
    ...
    toggleSign() {
        var displayBox = document.getElementById("displayTela");
    ...
    eraseLastInput() {
        ...
        document.getElementById("displayTela").value = recreatedOperation;
    ...
    calculateFactorial() {
        ...
        try {
            ...
            document.getElementById("displayTela").value = "Esse número é muito grande!";
        }
        this.clearDisplay();
        document.getElementById("displayTela").value = result;
    }
    ...
    nthTenPower() {
        var number = parseInt(this.operationString.split(new RegExp("[^0-9]")));
        this.clearDisplay();
        document.getElementById("displayTela").value = Math.pow(10, parseInt(number));
    }

    square() {
        var number = parseInt(this.operationString.split(new RegExp("[^0-9]")));
        this.clearDisplay();
        document.getElementById("displayTela").value = Math.pow(parseInt(number), 2);
    }

    cube() {
        var number = parseInt(this.operationString.split(new RegExp("[^0-9]")));
        this.clearDisplay();
        document.getElementById("displayTela").value = Math.pow(parseInt(number), 3);
    }

    inverseNumber() {
        var number = parseInt(this.operationString.split(new RegExp("[^0-9]")));
        this.clearDisplay();
        document.getElementById("displayTela").value = Math.pow(parseInt(number), -1);
    }
